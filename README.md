...
## Dependencies
PAL depends on [ArenaAllocator](github.com/eagletrt/libarena-allocator-sw.git) for memory management.
Ensure that you initialize the allocator handler before initializing the PAL handler.

Additionally, it requires [RingBuffer](github.com/eagletrt/libring-buffer-sw/tree/dev) for internal buffer management.
...
> communications done throught that handle are not guaranteed to always work in case of interrupts, an example implementation can be found in [RingBuffer's README](github.com/eagletrt/libring-buffer-sw)
...
- A way to allocate or provide buffers for incoming data (preferably using the ArenaAllocator).
